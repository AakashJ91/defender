...
class UIManager {
    ...
    handleCanvasPointer(e) {
        ...
        // Check if tapping an existing tower (Enlarged touch target)
        const clickedTower = this.engine.towers.find(t => Math.hypot(t.x - x, t.y - y) <= 42);
        if (clickedTower) {
    ...
    showTowerInfo(tower) {
        ...
        document.getElementById('inspect-level').textContent = `Tier ${tower.level}`;
